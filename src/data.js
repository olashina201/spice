export const NavData = [
    {
        title: 'Home',
        path:'/',
        cName: "navbar-list-items",
        active: "nav-link active"
    },
    {
        title: 'About Me',
        path:'/reports',
        cName: "navbar-list-items",
        active: "nav-link"
    },
    {
        title: 'Experience',
        path:'/login',
        cName: "navbar-list-items",
        active: "nav-link"
    },
    {
        title: 'My Work',
        path:'/team',
        cName: "navbar-list-items",
        active: "nav-link"
    },
    {
        title: 'Work With Me',
        path:'/messages',
        cName: "navbar-list-items",
        active: "nav-link"
    },
]

export const hero = {
  greet: "Hi there, I'm",
  name: "Jaden Tega",
  about:
    "An award-winning chef with over 12 years experience. Nulla vestibulum a lorem non luctus. Aliquam ut neque est",
};

export const about = {
  title: "About Me",
  content:
    "An award-winning chef with over 12 years experience Morbi porta turpis ac risus tempor mattis. Nulla vestibulum a lorem non luctus. Aliquam ut neque est. Sed tortor neque, rutrum dapibusviverra sit amet, tristique sed justo. Nulla maximus venenatis ligula quis cursus. Nullam laoreet luctus urna at mattis.",
};

export const services = {
  title: "Services I Offer",
  contentOne: {
    title: "Regular Personal Chef Services",
    content:
      "Morbi porta turpis ac risus tempor mattis. Nulla  vestibulum a lorem non luctus.",
  },
  contentTwo: {
    title: "Catering",
    content:
      "Morbi porta turpis ac risus tempor mattis. Nulla  vestibulum a lorem non luctus.",
  },
  contentThree: {
    title: "Cooking Classes",
    content:
      "Morbi porta turpis ac risus tempor mattis. Nulla  vestibulum a lorem non luctus.",
  },
  contentFour: {
    title: "New Kitchen Setup",
    content:
      "Morbi porta turpis ac risus tempor mattis. Nulla  vestibulum a lorem non luctus. ",
  },
  contentFive: {
    title: "Specialty Baking",
    content:
      "Morbi porta turpis ac risus tempor mattis. Nulla  vestibulum a lorem non luctus.",
  },
  contentSix: {
    title: "Meal Planning",
    content:
      "Morbi porta turpis ac risus tempor mattis. Nulla  vestibulum a lorem non luctus.",
  },
};

export const experiences = {
  experience: [
    {
      role: "JTSpice, CEO",
      date: "2016 - present",
      description:
        "Morbi porta turpis ac risus tempor mattis. Nulla vestibulum a lorem non luctus. Aliquam ut neque est. Sed tortor neque, rutrum dapibus viverra sit amet, tristique sed justo. Nulla maximus venenatis ligula quis cursus. Nullam laoreet luctus urna at mattis.",
    },
    {
      role: "Kentucky Fried Chicken, Chief Cook",
      date: "2012 - 2016",
      description:
        "Morbi porta turpis ac risus tempor mattis. Nulla vestibulum a lorem non luctus. Aliquam ut neque est. Sed tortor neque, rutrum dapibus viverra sit amet, tristique sed justo. Nulla maximus venenatis ligula quis cursus. Nullam laoreet luctus urna at mattis.",
    },
    {
      role: "Chillspot, Assistant Chef",
      date: "2005 - 2012",
      description:
        "Morbi porta turpis ac risus tempor mattis. Nulla vestibulum a lorem non luctus. Aliquam ut neque est. Sed tortor neque, rutrum dapibus viverra sit amet, tristique sed justo. Nulla maximus venenatis ligula quis cursus. Nullam laoreet luctus urna at mattis.",
    },
    {
      role: "Dallas Delight, Chief Cook",
      date: "2012 - 2014",
      description:
        "Morbi porta turpis ac risus tempor mattis. Nulla vestibulum a lorem non luctus. Aliquam ut neque est. Sed tortor neque, rutrum dapibus viverra sit amet, tristique sed justo. Nulla maximus venenatis ligula quis cursus. Nullam laoreet luctus urna at mattis.",
    },
  ],
};

export const testimony = [
    {
        id: 1, 
        image: 'https://demo.w3layouts.com/demos_new/template_demo/18-03-2021/study-course-starter-demo_Free/1612283260/web/assets/images/banner1.jpg',
        title: 'Be more productive to be more successful', 
        description: `Morbi porta turpis ac risus tempor mattis. Nulla vestibulum a lorem non luctus. Aliquam ut 
        neque est. Sed tortor neque, rutrum dapibus viverra sit amet, tristique sed justo. Nulla 
        maximus venen.`
    },
    {
        id: 2, 
        image: 'https://demo.w3layouts.com/demos_new/template_demo/18-03-2021/study-course-starter-demo_Free/1612283260/web/assets/images/banner2.jpg',
        title: 'Learn and improve yourself in less time', 
        description: 'Our self improvement course is very effective'
    },
    {
        id: 3, 
        image: 'https://demo.w3layouts.com/demos_new/template_demo/18-03-2021/study-course-starter-demo_Free/1612283260/web/assets/images/banner3.jpg',
        title: 'Enhance your skill with the best online course', 
        description: 'Take the first step to journey with us'
    },
    {
        id: 4, 
        image: 'https://demo.w3layouts.com/demos_new/template_demo/18-03-2021/study-course-starter-demo_Free/1612283260/web/assets/images/banner4.jpg',
        title: '50% discount on all popular courses', 
        description: 'Take the first step to journey with us'
    }
]
